...
1. **生成约束**：prompt 声明"只基于提供的块作答，引用标注 `[n]` 对应块序号"；未提供的知识一律不答。
...
| 级 | 时机 | 机制 | 初版参数 |
|---|---|---|---|
| L1 | 召回合并后 | 候选块最高分（RRF 分数/向量相似度）低于阈值 τ → 拒答 | τ 启发式初值，06 校准 |
| L2 | 生成后 | LLM 自检："回答内容是否全部有块支撑" | prompt 判定 + L1 规则兜底 |

**设计要点**：检索分数分布随库内容漂移，绝对阈值不可靠——τ 只是初版护栏（宁严勿松，产品原则"可信优先"），校准路径在 06（评估集上做阈值扫描）。L2 是 L1 漏网的二次闸：素材相关但答案编造（如模型把库外常识混入）由 L2 拦截。
