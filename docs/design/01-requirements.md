...
| 日期 | 2026-09-25 |
| 上游 | 无（根文档） |
| 变更 | v0.6：D4 明确单 worker 任务可持久化并在启动时恢复；v0.5：D1 增补 Markdown 本地图片包，明确图片只保留不解析；v0.4：PD1–PD4 拍板并入 D4–D7 |
...
| 排除项 | 理由 | 拟议版本 |
|---|---|---|
| PDF/Word、OCR 与图片语义解析 | 解析器是独立工程；当前只保留 Markdown 引用的本地原图，不理解图片内容 | V1.0 |
...
| US-M1-02 | 我想把一篇 Markdown 笔记导入指定知识库，必要时保留其中的本地图片 | `.md/.txt` 可直接上传；含图笔记以“一篇 `.md` + 被引用静态图片”的 ZIP 上传；返回文档 id 与处理状态 |
| US-M1-03 | 我想查看各库文档，知道库里有什么 | 按库过滤：标题、字数、更新时间、状态、块数与图片数；可查看完整原文和原始图片 |
...
```mermaid
flowchart TD
    A[用户上传 md/txt 或 Markdown 图片 ZIP] --> B{校验: 格式/大小/内容/包完整性}
...
### D1（已拍板）：一期索引内容仅 Markdown / 纯文本

- **决策**：`.md` / `.txt` 直接导入；含本地图片时，用 ZIP 携带恰好一篇 `.md` 及其相对路径引用的静态 PNG/JPEG/WebP。索引与 embedding 仍只处理 Markdown 文本，图片按原始字节保存和展示。
- **备选**：直接支持 PDF/Word——解析引入独立失败域，拖慢主链路验证；扫描件涉及 OCR 是另一工程。
- **理由**：Markdown 的结构化文本让切块天然对齐语义块，检索质量上限更高。
- **代价**：图片内容不可被搜索，只有 alt 文本与周边 Markdown 参与检索；PDF/Word 存量笔记仍需先转 Markdown。
...
- **理由**：NFR 的性能基线是单用户操作，进程内执行完全覆盖；状态表保证阶段可见、版本化原子领取和单 worker 重启续跑，职责在 M2。
- **代价**：执行器仍没有多 worker 租约、心跳和跨进程抢占；若未来做多实例部署需更换承载，但任务表和 API 状态契约可以保留。
