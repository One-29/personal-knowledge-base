...
$evalDatabasePath = Join-Path $projectRoot "data\eval\knowbase-eval.db"
$evalDatabaseUrl = if ($Backend -eq "SQLite") {
    "sqlite+pysqlite:///" + $evalDatabasePath.Replace("\", "/")
}
else {
    "postgresql+psycopg://postgres:postgres@127.0.0.1:5432/knowbase_eval"
}
...
if ($Backend -eq "PostgreSQL" -and -not (Test-Path -LiteralPath $dockerModule -PathType Leaf)) {
...
try {
    ...
    if ($Backend -eq "PostgreSQL") {
        ...
        Write-Stage "Using isolated SQLite evaluation file $evalDatabasePath"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($ReferenceReportPath)) {
        $evaluationArguments += @("--reference-report", $ReferenceReportPath)
    }
